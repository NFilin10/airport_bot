...
from main import get_country_indexes, sihtkohad, kuupaev_kontroll, get_tickets_link, get_dest_airport_name, get_best_ticket_prices
import keyboard
...
def vale_kuupaev(message, sihtkoht, suund):
        bot.send_message(message.chat.id, "Valige kuupäev", reply_markup=keyboard.delete_keyboard)
        bot.register_next_step_handler(message, date, suund, sihtkoht)
...
def date(message, suund, sihtkoht):
    ...
    #    bot.register_next_step_handler(message, link_vajalik_vastus, sihtkoht, user_date, back_date(message, sihtkoht),)
...
def link_vajalik_vastus(message, sihtkoht, user_date, suund, tagasilend, suund1):
    vastus = message.text
    if vastus == "Jah":
        #get_tickets_link(sihtkoht, kuupaev, tagasi_lend, suund, adults, children, pens)
        bot.send_message(message.chat.id, "Sisestage taiskasvanute arv")
...
def lapsed(message, sihtkoht, user_date, suund, tagasilend, adults, suund1):
    ...
    print("lp", children)
    bot.send_message(message.chat.id, "Siestage pensionaaride arv")
...
def imikud(message, sihtkoht, user_date, suund, tagasilend, adults, children, suund1):
    ...
    print("in", infants)
    bot.send_message(message.chat.id, f"See on Teie piletite link: {get_tickets_link(sihtkoht, user_date, suund, tagasilend, adults, children, infants, suund1)}")
    dest_airport_name = get_dest_airport_name(sihtkoht)
    dest_airport_name = dest_airport_name.replace("(", "").replace(")", "").split(' ')[1]

    print(dest_airport_name)
    print(get_best_ticket_prices(user_date, tagasilend, adults, children, infants, dest_airport_name, suund1))
